...
class WFS_Account {
	...
	public static function menu_item( $items ) {
		...
		$items[ self::ENDPOINT ] = __( 'Subscriptions', 'webifya-subscriptions' );
		if ( null !== $logout ) {
	...
	public static function content() {
		...
		if ( ! $subscriptions ) {
			echo '<p>' . esc_html__( 'You do not have any subscriptions yet.', 'webifya-subscriptions' ) . '</p>';
			return;
		}

		echo '<table class="woocommerce-orders-table shop_table shop_table_responsive">';
		echo '<thead><tr><th>' . esc_html__( 'Subscription', 'webifya-subscriptions' ) . '</th><th>' . esc_html__( 'Status', 'webifya-subscriptions' ) . '</th><th>' . esc_html__( 'Next payment', 'webifya-subscriptions' ) . '</th><th>' . esc_html__( 'Actions', 'webifya-subscriptions' ) . '</th></tr></thead><tbody>';
		foreach ( $subscriptions as $subscription ) {
			...
			if ( $pending && $pending->needs_payment() ) {
				echo '<a class="button pay" href="' . esc_url( $pending->get_checkout_payment_url() ) . '">' . esc_html__( 'Pay renewal', 'webifya-subscriptions' ) . '</a> ';
			}
			if ( in_array( $status, array( 'active', 'trialling', 'past-due', 'on-hold' ), true ) ) {
				echo '<a class="button cancel" href="' . esc_url( $cancel ) . '">' . esc_html__( 'Cancel', 'webifya-subscriptions' ) . '</a>';
			}
			echo '</td></tr>';
		}
		echo '</tbody></table>';
	...
	public static function handle_cancel() {
		...
		if ( $owner !== get_current_user_id() || ! wp_verify_nonce( $nonce, 'wfs_cancel_' . $subscription_id ) ) {
			wc_add_notice( __( 'This subscription could not be cancelled.', 'webifya-subscriptions' ), 'error' );
		...
		wc_add_notice( __( 'Your subscription has been cancelled.', 'webifya-subscriptions' ) );
		wp_safe_redirect( wc_get_account_endpoint_url( self::ENDPOINT ) );
		exit;
	}
}
